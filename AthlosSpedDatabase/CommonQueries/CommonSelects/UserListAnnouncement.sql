...
-- SELECT ALL INDIVIDUAL CUSTOMERS
...
INNER JOIN dbo.District AS D ON D.DistrictID = U.DistrictID
INNER JOIN dbo.Customers AS C ON C.CustomerID = D.CustomerID
WHERE C.CustomerIsCurrent = 1 AND U.DistrictID <> 1 AND U.DistrictID <> 7 AND (U.UserIsCurrent = 1 OR AspNetUserID IS NULL) AND CustomerLicenseType = 'Individual'
ORDER BY
	C.CustomerLicenseType
	,D.DistrictName
	,CASE
